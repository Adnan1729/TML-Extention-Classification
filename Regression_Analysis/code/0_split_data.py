...
def split_and_save_dataset(dataset, perc = 0.3):
    ...
    df = pd.read_csv(f'C:\\Users\\AMahmud\\Regression_Analysis\\input\\base_raw\\{dataset}.csv', index_col=0, header=0)
    train, test = train_test_split(df, test_size = perc)
    train.to_csv(f'C:\\Users\\AMahmud\\Regression_Analysis\\input\\base_processed\\{dataset}_train.csv')
    test.to_csv(f'C:\\Users\\AMahmud\\Regression_Analysis\\input\\base_processed\\{dataset}_test.csv')
